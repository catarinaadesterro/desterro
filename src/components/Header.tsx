      ...
      {NAV_ITEMS.map(item => {
        ...
        return (
          <Link key={item.href} href={item.href} style={{
            color: isActive ? 'var(--blue)' : 'var(--paper)',
            textDecoration: 'none',
            fontFamily: item.italic ? 'var(--serif)' : 'var(--mono)',
            fontStyle: item.italic ? 'italic' : 'normal',
            fontSize: item.italic ? 13 : 10,
            letterSpacing: item.italic ? 0 : '.18em',
            textTransform: item.italic ? 'none' : 'uppercase',
            whiteSpace: 'nowrap',
          }}>
